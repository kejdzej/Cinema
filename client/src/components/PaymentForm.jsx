...
function InnerPaymentForm({ clientSecret, onSuccess, amountPln }){
  ...
  return (
    <div className="card" style={{maxWidth: 520}}>
      ...
      <p style={{opacity:.8, marginBottom: 12}}>Do zapłaty: <b>{amountPln.toFixed(2)} zł</b></p>
      <PaymentElement />
      {error && <div className="toast error" style={{marginTop: 12}}>{error}</div>}
      <button className="btn" onClick={handlePay} disabled={!stripe || loading} style={{marginTop: 12}}>
